...
| T3.1–T3.4 | 💳 Payments — complete end-to-end UPI payment flow |
| — | — API: submit-payment, admin verify/reject, shop settings CRUD, PaymentDto/SubmitPaymentBody/VerifyPaymentBody schemas, `toPaymentDto` mapper, `upiQrUrl` on ShopSettingsDto |
| — | — Mobile screens: Admin settings (A14) — UPI VPA, payee name, QR URL, delivery fee, COD limit, accepting orders toggle. Admin payments queue (A04) — pending payment list with Verify/Reject buttons, cursor pagination, query invalidation on success. UPI payment screen (C09) — QR hero, copy-UPI-ID, GPay/PhonePe/Paytm deep links with generic fallback, UTR input (6-18 alphanumeric), submit with PAYMENT_ALREADY_SUBMITTED handling, "Pay Cash Instead" fallback |
