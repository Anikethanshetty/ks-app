...
| T2.2 | ✅ Checkout flow (C08) — API: address CRUD (`GET/POST/PATCH/DELETE /addresses`) with ownership scoping, `GET /shop/settings` (delivery fee, COD limit, radius, shop name for any authenticated role), `GET /orders/preview?addressId=X` (validates cart + address ownership, computes bill from current cart items, checks stock, COD limit, delivery fee). Rate limit raised to 10K in non-production to avoid authz test 429s. Mobile: address management screen (list, add, edit, delete with form validation), checkout screen with horizontal-scroll address picker (auto-selects default), bill summary (items, subtotal, delivery fee, total with free-delivery hint), COD limit warning, out-of-stock warning, place order button with error-code-specific alerts, order success screen stub. Full kn/hi/en i18n. Authz tests: 11 new (address CRUD control + cross-customer 404 ×2 + delivery 403 + no token 401 + shop settings control + no token 401 + order preview control + cross-customer INVALID_ADDRESS). Typecheck clean, 70 authz tests pass (4 todo). |

**Phase 0 done** (T0.1–T0.6), committed (`1ba72ee`). **T1.1 done**, committed (`a011ff0`). **T1.2 done**, committed (`eb0255d`). **T1.3 done**, committed (`b0255ce`). **T1.4 done**, committed (`88248a2`). **T1.5 done**, committed (`58930b6`). **T1.6 done**, committed (`bdab3f3`). **T2.1 done**, committed (`db443a5`). **T2.2 done**, committed (`61070dd`).

## Next

**Phase 2 — Manual ordering** (`06-implementation-plan.md`), next up:
- **T2.3** — Wire place_order with full error handling (OUT_OF_STOCK, COD_LIMIT_EXCEEDED, SHOP_NOT_ACCEPTING_ORDERS, etc.) — already partially done in T2.2 checkout.
- **T2.4** — Order success screen (C10) with detail + status timeline, My Orders list (C11) — success screen stub exists, needs full timeline.
- **T2.5** — Admin order board (A01) with live tabs, sound alert on new order.
- **T2.6** — Realtime + push via Socket.IO with server-decided rooms.
