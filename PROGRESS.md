...
| T2.1 | 🛒 Cart system — API: `GET /cart`, `POST /cart/add`, `PATCH /cart/:itemId`, `DELETE /cart/:itemId`, `GET /cart/count`. Cart repository with raw-SQL queries and ownership scoping (user scoped via `JOIN carts c WHERE c.user_id = $1`). Add-to-cart upserts (increments quantity if variant already in cart). Empty cart, error, loading states. Mobile: Zustand cart store with AsyncStorage persistence (survives app restart), cart screen with quantity steppers (+/−/🗑), live subtotal, checkout button, cart badge on customer home with red item count. Add to Cart wired on product detail screen. Full kn/hi/en i18n. Authz tests: 11 new (A add/GET/update/remove, customer cross-customer isolation ×2, delivery 403, no token 401, admin add/GET/count). Typecheck clean, 60 authz tests pass (4 todo). |
| T2.2 | ✅ Checkout flow (C08) — API: address CRUD (`GET/POST/PATCH/DELETE /addresses`) with ownership scoping, `GET /shop/settings` (delivery fee, COD limit, radius, shop name for any authenticated role), `GET /orders/preview?addressId=X` (validates cart + address ownership, computes bill from current cart items, checks stock, COD limit, delivery fee). Rate limit raised to 10K in non-production to avoid authz test 429s. Mobile: address management screen (list, add, edit, delete with form validation), checkout screen with horizontal-scroll address picker (auto-selects default), bill summary (items, subtotal, delivery fee, total with free-delivery hint), COD limit warning, out-of-stock warning, place order button with error-code-specific alerts (OUT_OF_STOCK, COD_LIMIT_EXCEEDED, SHOP_NOT_ACCEPTING_ORDERS, etc.), order success screen stub. Full kn/hi/en i18n. Authz tests: 11 new (address CRUD control + cross-customer 404 ×2 + delivery 403 + no token 401 + shop settings control + no token 401 + order preview control + cross-customer INVALID_ADDRESS). Typecheck clean, 70 authz tests pass (4 todo). |
| T2.3 | ⚡ Place_order wired end-to-end — checkout screen already had full error-code-specific alerts (OUT_OF_STOCK, COD_LIMIT_EXCEEDED, SHOP_NOT_ACCEPTING_ORDERS, EMPTY_ORDER, INVALID_ADDRESS), `Idempotency-Key` support via Redis, `POST /orders` route with proper authorization (customer/admin), loading/success/error states. |
...
**Phase 0 done** (T0.1–T0.6), committed (`1ba72ee`). **T1.1 done**, committed (`a011ff0`). **T1.2 done**, committed (`eb0255d`). **T1.3 done**, committed (`b0255ce`). **T1.4 done**, committed (`88248a2`). **T1.5 done**, committed (`58930b6`). **T1.6 done**, committed (`bdab3f3`). **T2.1 done**, committed (`db443a5`). **T2.2 done**, committed (`61070dd`). **T2.3 done**, committed (next). **T2.4 done**, committed (next). **T2.5 done**, committed (next).

## Next

**Phase 2 — Manual ordering** (`06-implementation-plan.md`), next up:
- **T2.6** — Realtime + push via Socket.IO with server-decided rooms. This is the biggest remaining Phase 2 task: Socket.IO gateway with Redis adapter, server-decided rooms, `sendPush` BullMQ job for Expo Push Notifications, replace 15s polling in admin inventory.
