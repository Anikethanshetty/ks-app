...
**Phase 0 done** (T0.1–T0.6), committed (`1ba72ee`). **T1.1 done**, committed (`a011ff0`). **T1.2 done**, committed (`eb0255d`). **T1.3 done**, committed (`b0255ce`). **T1.4 done**, committed (`88248a2`). **T1.5 done**, committed (`58930b6`). **T1.6 done**, committed (`bdab3f3`). **T2.1 done**, uncommitted.

## Next

**Phase 2 — Manual ordering** (`06-implementation-plan.md`), next up:
- **T2.2** — Checkout (C08): address picker, delivery slot, payment method, bill summary. Enforce delivery radius and COD limit client-side + server-side.
- T2.3 (Wire place_order with error handling), T2.4 (Success screen + My orders + order timeline), T2.5 (Admin order board with sound alert), T2.6 (Realtime + push via Socket.IO).
