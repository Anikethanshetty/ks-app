...
CsvTest Dal,ಸಿಎಸ್ವಿ ಟೆಸ್ಟ್ ಬೇಳೆ,सीएसवी टेस्ट दाल,TstCat,1 kg,kg,120,110,30`;
  ...
  it("any authenticated role can register a device token (control)", async () => {
    const customerRes = await app.inject({
      method: "POST",
    ...
    expect(customerRes.statusCode).toBe(200);
    expect(customerRes.json().ok).toBe(true);

    const adminRes = await app.inject({
      method: "POST",
      url: `${V1}/device-tokens`,
      headers: auth(ADMIN.accessToken),
      payload: { token: "ExponentPushToken[yyyyyyyyyyyyyyyyyyyyyy]", platform: "android" },
    });
    expect(adminRes.statusCode).toBe(200);
  });

  it("no token on device token registration → 401", async () => {
    const res = await app.inject({
      method: "POST",
      url: `${V1}/device-tokens`,
      payload: { token: "ExponentPushToken[zzzzzzzzzzzzzzzzzzzzzz]" },
    });
    expect(res.statusCode).toBe(401);
  });

  it("user can unregister their own device token (control)", async () => {
    const res = await app.inject({
      method: "DELETE",
      url: `${V1}/device-tokens/ExponentPushToken[xxxxxxxxxxxxxxxxxxxxxx]`,
      headers: auth(A.accessToken),
    });
    expect(res.statusCode).toBe(200);
  });

  it("B cannot unregister A's device token (silent no-op)", async () => {
    const res = await app.inject({
      method: "DELETE",
      url: `${V1}/device-tokens/ExponentPushToken[yyyyyyyyyyyyyyyyyyyyyy]`,
      headers: auth(B.accessToken),
    });
    expect(res.statusCode).toBe(200);
    // Ensure the token still exists (owned by ADMIN, not B)
    const token = await prisma.deviceToken.findUnique({
      where: { token: "ExponentPushToken[yyyyyyyyyyyyyyyyyyyyyy]" },
    });
